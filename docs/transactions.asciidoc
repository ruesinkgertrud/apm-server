...
A transaction can for example be an HTTP request or an asynchronous background job.
...
If sampled,
the <<transaction-spans,spans>> of a transaction are sent and stored separate documents.
